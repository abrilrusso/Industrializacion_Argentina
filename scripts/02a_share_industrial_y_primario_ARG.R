...
primario_arg <- prim_arg %>%
  select(anio, valor) %>%
  mutate(sector = "Sector primario")

subperiodos <- c(1970, 1990, 2003, 2011, 2024)

datos <- bind_rows(share_arg, primario_arg) %>%
...
g <- ggplot(datos, aes(anio, valor, fill = sector)) +
  ...
  scale_fill_manual(
    values = c("Industria" = "#4FA8D8", "Sector primario" = "#E8C547"),
    name   = NULL
  ) +
  ...
  labs(
    title    = "Industria y sector primario en el PBI de Argentina por subperíodo",
    subtitle = "Participación en el PBI (%) | celeste = industria | amarillo = sector primario",
    x        = NULL,
    y        = "% del PBI",
...
ggsave("output/02a_share_industrial_y_primario_ARG.png", g, width = 9, height = 5.5, dpi = 300, bg = "white")
